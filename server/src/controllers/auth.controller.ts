...
import { db } from '../db';
import { users, refreshTokens, emailVerificationCodes, passwordResetTokens, clientApps } from '../db/schema';
...
export const login = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // OAuth flow: return redirect URL instead of server-side redirect (CSRF-safe AJAX flow)
    if (client_id && redirect_uri) {
      ...
      const authorizeUrl = `${baseUrl}/api/v1/oauth/authorize?${params.toString()}`;
      res.status(200).json({
        success: true,
